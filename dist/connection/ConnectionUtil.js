"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const mongoose_1 = require("mongoose");
const user = 'admin';
const pwd = 'admin';
const db = 'test?retryWrites=true';
const url = '@powerfull-api-mongodb-cluster-gbmsd.mongodb.net/';
const connectionString = `mongodb+srv://${user}:${pwd}${url}${db}`;
const ConnectionUtil = {
    mongooseConnect: () => mongoose_1.connect(connectionString, { useNewUrlParser: true })
        .catch(err => console.error('could not connect to database: ', err)),
    mongooseConnection: () => mongoose_1.connection.once('open', () => {
        console.log('Connected to database');
    })
};
exports.default = ConnectionUtil;
